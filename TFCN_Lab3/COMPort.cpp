...
void COMPort::read()
{
    ...
    if (PacketManager::checkForErrors(unstuffed))
    {
        window->ui->debugEdit->append("This packet has error.");
        PacketManager::correctError(unstuffed);
        window->ui->debugEdit->append("After correction:");
        window->ui->debugEdit->append(PacketManager::toHexString(unstuffed));
    }
